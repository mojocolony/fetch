...
function areaLabel(area){return area||'Unsorted'}
function areaColor(area){if(!area)return '#8a9196';return AREA_COLORS[area]||COLOR_POOL[hash(area)%COLOR_POOL.length]}
...
function normalizeArea(value){const area=String(value||'').trim().replace(/\s+/g,' ');return area.toLocaleLowerCase()==='unsorted'?'':area}
...
class LocalStore{
 ...
 async renameArea(oldName,newName){const all=(await this.list()).map(x=>x.category===oldName?{...x,category:newName}:x);localStorage.setItem(STORAGE_KEY,JSON.stringify(all));return all.filter(x=>x.category===newName).length}
 async renameTag(oldName,newName){const oldNorm=oldName.toLocaleLowerCase(),newNorm=newName.toLocaleLowerCase();let affected=0;const all=(await this.list()).map(x=>{const tags=x.tags||[];if(!tags.some(t=>t.toLocaleLowerCase()===oldNorm))return x;affected++;const next=[];for(const tag of tags){const candidate=tag.toLocaleLowerCase()===oldNorm?newName:tag;if(!next.some(t=>t.toLocaleLowerCase()===candidate.toLocaleLowerCase()))next.push(candidate)}return {...x,tags:next}});localStorage.setItem(STORAGE_KEY,JSON.stringify(all));return affected}
}
class SupabaseStore{
 ...
 async renameArea(oldName,newName){const {data,error}=await this.client.rpc('fetch_rename_area',{old_name:oldName,new_name:newName});if(error)throw error;return data||0}
 async renameTag(oldName,newName){const {data,error}=await this.client.rpc('fetch_rename_tag',{old_name:oldName,new_name:newName});if(error)throw error;return data||0}
}
...
function allTags(area=null){const pool=area===UNSORTED?items.filter(i=>!i.category):area?items.filter(i=>i.category===area):items;return [...new Set(pool.flatMap(i=>i.tags||[]))].sort((a,b)=>a.localeCompare(b))}
...
function buildNav(){
 const areas=allAreas(),unsortedCount=items.filter(i=>!i.category).length;$('countAll').textContent=items.length;$('countRecent').textContent=items.filter(i=>dayDiff(i.saved_at)<=7).length;$('countStarred').textContent=items.filter(i=>i.starred).length;
 const unsorted=`<button class="nav-item area-nav ${state.area===UNSORTED?'active':''}" data-area="${UNSORTED}" style="--area-tint:rgba(138,145,150,.12)"><span class="nav-left"><span class="area-dot" style="background:#8a9196"></span><span>Unsorted</span></span><span class="nav-count">${unsortedCount}</span></button>`;
 $('areaNav').innerHTML=unsorted+areas.map(area=>{const c=areaColor(area);const count=items.filter(i=>i.category===area).length;return `<button class="nav-item area-nav ${state.area===area?'active':''}" data-area="${esc(area)}" style="--area-tint:${hexToRgba(c,.12)}"><span class="nav-left"><span class="area-dot" style="background:${c}"></span><span>${esc(area)}</span></span><span class="nav-count">${count}</span></button>`}).join('');
 document.querySelectorAll('[data-area]').forEach(b=>b.addEventListener('click',()=>{exitItemView();state.area=b.dataset.area;state.special='all';state.tag=null;state.domain='';state.date=null;state.pageDate=null;state.type=null;syncControls();render();closeMobileNav()}));
...
function syncControls(){
 const areas=allAreas();$('filterArea').innerHTML=`<option value="">Any area</option><option value="${UNSORTED}">Unsorted</option>`+areas.map(a=>`<option value="${esc(a)}">${esc(a)}</option>`).join('');$('filterArea').value=state.area||'';
...
function getFiltered(){
 let rows=[...items];if(state.itemId)return rows.filter(i=>String(i.id)===String(state.itemId));if(state.special==='recent')rows=rows.filter(i=>dayDiff(i.saved_at)<=7);if(state.special==='starred')rows=rows.filter(i=>i.starred);if(state.area)rows=rows.filter(i=>state.area===UNSORTED?!i.category:i.category===state.area);if(state.tag)rows=rows.filter(i=>(i.tags||[]).includes(state.tag));if(state.domain)rows=rows.filter(i=>i.domain===state.domain);if(state.date)rows=rows.filter(i=>{const d=dayDiff(i.saved_at);return state.date==='today'?d<=1:state.date==='week'?d<=7:state.date==='month'?d<=30:d<=90});if(state.pageDate)rows=rows.filter(i=>pageDateMatches(i.page_date,state.pageDate));if(state.type)rows=rows.filter(i=>i.capture_type===state.type);
 const q=state.query.trim().toLocaleLowerCase();if(q)rows=rows.filter(i=>[i.title,i.domain,areaLabel(i.category),i.note,i.selected_text,...(i.tags||[])].filter(Boolean).join(' ').toLocaleLowerCase().includes(q));
 const cmp={newest:(a,b)=>new Date(b.saved_at)-new Date(a.saved_at),oldest:(a,b)=>new Date(a.saved_at)-new Date(b.saved_at),'page-newest':(a,b)=>(b.page_date||'').localeCompare(a.page_date||''),'page-oldest':(a,b)=>(a.page_date||'9999').localeCompare(b.page_date||'9999'),domain:(a,b)=>a.domain.localeCompare(b.domain),area:(a,b)=>areaLabel(a.category).localeCompare(areaLabel(b.category)),title:(a,b)=>a.title.localeCompare(b.title)}[state.sort];return rows.sort(cmp||(()=>0));
}
function titleForView(){if(state.itemId)return'Saved item';if(state.area)return state.area===UNSORTED?'Unsorted':state.area;if(state.special==='recent')return'Recent';if(state.special==='starred')return'Starred';return'Everything'}
function cardHTML(item){const areaName=areaLabel(item.category),color=areaColor(item.category),tint=hexToRgba(color,.12);const tags=(item.tags||[]).map(t=>`<button class="tag-chip" data-tag="${esc(t)}">${esc(t)}</button>`).join('');const shot=item.screenshot_url?`<img src="${esc(item.screenshot_url)}" alt="Saved viewport of ${esc(item.title)}" loading="lazy">`:`<div class="shot-fallback">${esc(item.domain||'Saved page')}</div>`;return `<article class="item-card" data-id="${esc(item.id)}" data-open="${esc(item.url)}" tabindex="0" aria-label="Open ${esc(item.title)}"><div class="shot-wrap" data-preview="${esc(item.id)}">${shot}<span class="type-pill">${esc(item.capture_type||'Page')}</span><div class="card-actions"><button class="more-button" data-more="${esc(item.id)}" aria-label="Bookmark actions" title="Bookmark actions"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 6h16"/><path d="M4 12h16"/><path d="M4 18h16"/></svg></button><button class="star-button ${item.starred?'is-starred':''}" data-star="${esc(item.id)}" aria-label="${item.starred?'Unstar':'Star'}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m12 2 3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01z"/></svg></button></div></div><div class="item-menu" data-menu="${esc(item.id)}"><button data-action="edit" data-item="${esc(item.id)}">Edit bookmark</button><button data-action="copy" data-item="${esc(item.id)}">Copy Fetch link</button><button data-action="share" data-item="${esc(item.id)}">Share…</button><button class="danger" data-action="delete" data-item="${esc(item.id)}">Delete</button></div><div class="card-body"><h3 class="item-title">${esc(item.title)}</h3><div class="item-meta"><div class="domain-line"><span class="area-dot" style="background:${color}"></span><span>${esc(item.domain)}</span></div><span class="page-date">${item.page_date?fmtPage(item.page_date):fmtSaved(item.saved_at)}</span></div><div class="tags-row"><span class="area-chip" style="--area-tint:${tint}"><span class="area-dot" style="background:${color}"></span>${esc(areaName)}</span>${tags}</div></div></article>`}
function updateResultText(rows=getFiltered()){
 const single=!!state.itemId;
 $('resultText').textContent=single?(rows.length?'Direct Fetch link · 1 saved item':'Direct Fetch link'):`${rows.length} item${rows.length===1?'':'s'}`;
}
function updateStarCount(){$('countStarred').textContent=items.filter(i=>i.starred).length}
function applyStarButtonState(btn,starred){if(!btn)return;btn.classList.toggle('is-starred',starred);btn.setAttribute('aria-label',starred?'Unstar':'Star')}
async function toggleStar(item,btn){
 const previous=!!item.starred;const next=!previous;item.starred=next;applyStarButtonState(btn,next);updateStarCount();
 try{
  await store.update(item.id,{starred:next});
  if(state.special==='starred'&&!next&&!state.itemId){
   const card=btn.closest('.item-card');if(card){card.classList.add('removing');setTimeout(()=>{card.remove();const rows=getFiltered();updateResultText(rows);$('empty').classList.toggle('show',!rows.length);$('items').style.display=rows.length?'':'none'},140)}
  }
 }catch(err){
  console.error(err);item.starred=previous;applyStarButtonState(btn,previous);updateStarCount();showNotice(err.message||'Could not update star.',5000);
 }
}
function render(){
 const rows=getFiltered();const single=!!state.itemId;document.body.classList.toggle('single-item-mode',single);$('backToLibrary').hidden=!single;$('viewTitle').textContent=titleForView();updateResultText(rows);$('items').classList.toggle('single-item-view',single);$('items').innerHTML=rows.map(cardHTML).join('');$('empty').textContent=single?'This Fetch item could not be found.':'Nothing matches those filters.';$('empty').classList.toggle('show',!rows.length);$('items').style.display=rows.length?'':'none';
...
function renderSettingsManager(){
 const areas=allAreas(),tags=allTags();
 const areaSelect=$('manageAreaSelect'),tagSelect=$('manageTagSelect');
 if(areaSelect){const previous=areaSelect.value;areaSelect.innerHTML=areas.length?areas.map(a=>`<option value="${esc(a)}">${esc(a)}</option>`).join(''):'<option value="">No named areas yet</option>';areaSelect.disabled=!areas.length;areaSelect.value=areas.includes(previous)?previous:(areas[0]||'');$('renameAreaInput').value=areaSelect.value||'';$('renameAreaBtn').disabled=!areas.length}
 if(tagSelect){const previous=tagSelect.value;tagSelect.innerHTML=tags.length?tags.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join(''):'<option value="">No tags yet</option>';tagSelect.disabled=!tags.length;tagSelect.value=tags.includes(previous)?previous:(tags[0]||'');$('renameTagInput').value=tagSelect.value||'';$('renameTagBtn').disabled=!tags.length}
}
async function renameAreaFromSettings(){const oldName=$('manageAreaSelect').value,newName=$('renameAreaInput').value.trim();if(!oldName||!newName)return showNotice('Choose an Area and enter its new name.');if(newName.toLocaleLowerCase()==='unsorted')return showNotice('Unsorted is automatic. Leave an item’s Area blank instead.');if(oldName===newName)return showNotice('That Area already has that name.');const target=allAreas().find(a=>a.toLocaleLowerCase()===newName.toLocaleLowerCase()&&a.toLocaleLowerCase()!==oldName.toLocaleLowerCase());if(target&&!confirm(`“${target}” already exists. Merge “${oldName}” into it?`))return;const btn=$('renameAreaBtn');btn.disabled=true;try{await store.renameArea(oldName,target||newName);if(state.area===oldName)state.area=target||newName;await refresh();renderSettingsManager();showNotice(target?`Merged “${oldName}” into “${target}”.`:`Area renamed to “${newName}”.`)}catch(err){console.error(err);showNotice(err.message||'Could not rename this Area.',5000)}finally{btn.disabled=false}}
async function renameTagFromSettings(){const oldName=$('manageTagSelect').value,newName=$('renameTagInput').value.trim();if(!oldName||!newName)return showNotice('Choose a Tag and enter its new name.');if(oldName===newName)return showNotice('That Tag already has that name.');const target=allTags().find(t=>t.toLocaleLowerCase()===newName.toLocaleLowerCase()&&t.toLocaleLowerCase()!==oldName.toLocaleLowerCase());if(target&&!confirm(`“${target}” already exists. Merge “${oldName}” into it?`))return;const btn=$('renameTagBtn');btn.disabled=true;try{await store.renameTag(oldName,target||newName);if(state.tag===oldName)state.tag=target||newName;await refresh();renderSettingsManager();showNotice(target?`Merged “${oldName}” into “${target}”.`:`Tag renamed to “${newName}”.`)}catch(err){console.error(err);showNotice(err.message||'Could not rename this Tag.',5000)}finally{btn.disabled=false}}
function goToEverything(){exitItemView();state.area=null;state.tag=null;state.domain='';state.date=null;state.pageDate=null;state.type=null;state.special='all';syncControls();render();closeMobileNav()}
function wireMobileGestures(){let gesture=null;const isMobile=()=>window.matchMedia('(max-width:900px)').matches;document.addEventListener('touchstart',e=>{if(!isMobile()||e.touches.length!==1)return;if(document.querySelector('.backdrop.show')||$('filterPanel')?.classList.contains('show'))return;const t=e.touches[0],drawerOpen=document.body.classList.contains('mobile-nav-open'),standalone=window.matchMedia('(display-mode: standalone)').matches||navigator.standalone===true,minX=standalone?0:22,maxX=standalone?58:90;if(!drawerOpen&&(t.clientX<minX||t.clientX>maxX))return;gesture={x:t.clientX,y:t.clientY,time:Date.now(),drawerOpen}}, {passive:true});document.addEventListener('touchend',e=>{if(!gesture||!e.changedTouches.length)return;const t=e.changedTouches[0],dx=t.clientX-gesture.x,dy=t.clientY-gesture.y,duration=Date.now()-gesture.time,start=gesture;gesture=null;if(duration>700||Math.abs(dx)<64||Math.abs(dx)<Math.abs(dy)*1.2)return;if(start.drawerOpen&&dx<0){setMobileNav(false);return}if(!start.drawerOpen&&dx>0){if(state.itemId)goToEverything();else setMobileNav(true)}}, {passive:true});document.addEventListener('touchcancel',()=>{gesture=null},{passive:true})}
...
function openCapture(){$('captureUrl').value='';$('captureTitle').value='';$('areaInput').value=state.area&&state.area!==UNSORTED?state.area:'';$('tagInput').value='';$('captureType').value='Page';$('captureNote').value='';syncControls();$('captureBackdrop').classList.add('show');setTimeout(()=>$('captureUrl').focus(),0)}
async function saveManualCapture(){const url=$('captureUrl').value.trim(),title=$('captureTitle').value.trim(),area=normalizeArea($('areaInput').value),tags=normalizeTags($('tagInput').value);if(!url||!title){showNotice('URL and title are required.');return}const domain=domainFrom(url);if(!domain){showNotice('That URL does not look valid.');return}const type=$('captureType').value;const item={id:crypto.randomUUID(),title,url,domain,category:area||null,saved_at:new Date().toISOString(),page_date:null,capture_type:type,starred:false,note:$('captureNote').value.trim(),selected_text:type==='Text'?$('captureNote').value.trim()||null:null,image_url:null,screenshot_path:null};try{await store.save(item,tags);$('captureBackdrop').classList.remove('show');await refresh();showNotice('Saved to Fetch.')}catch(err){console.error(err);showNotice(err.message||'Could not save this item.',5000)}}
...
function wireUI(){
 const mobileNavBtn=$('mobileNavBtn'),mobileNavClose=$('mobileNavClose'),mobileNavBackdrop=$('mobileNavBackdrop');wireMobileGestures();if(mobileNavBtn)mobileNavBtn.addEventListener('click',()=>setMobileNav(!document.body.classList.contains('mobile-nav-open')));if(mobileNavClose)mobileNavClose.addEventListener('click',()=>setMobileNav(false));if(mobileNavBackdrop)mobileNavBackdrop.addEventListener('click',()=>setMobileNav(false));window.addEventListener('resize',()=>{if(!window.matchMedia('(max-width:900px)').matches)setMobileNav(false)});
 document.querySelectorAll('[data-special]').forEach(b=>b.addEventListener('click',()=>{exitItemView();state.area=null;state.tag=null;state.domain='';state.date=null;state.pageDate=null;state.type=null;state.special=b.dataset.special;syncControls();render();closeMobileNav()}));
 $('backToLibrary').addEventListener('click',goToEverything);
 ...
 $('accountBtn').addEventListener('click',()=>{closeMobileNav();updateAccountUI();$('accountBackdrop').classList.add('show')});$('cancelAccount').addEventListener('click',()=>close('accountBackdrop'));$('closeAccount').addEventListener('click',()=>close('accountBackdrop'));$('signInBtn').addEventListener('click',signInWithPassword);$('sendMagicLink').addEventListener('click',sendMagicLink);$('signOutBtn').addEventListener('click',async()=>{await supabaseClient.auth.signOut();close('accountBackdrop')});
 $('deviceBtn').addEventListener('click',async()=>{closeMobileNav();updateAccountUI();$('deviceBackdrop').classList.add('show');await refreshDevices()});$('closeDevice').addEventListener('click',()=>close('deviceBackdrop'));$('generateDeviceToken').addEventListener('click',generateDeviceToken);$('copyDeviceToken').addEventListener('click',async()=>{await navigator.clipboard.writeText($('deviceToken').value);showNotice('Token copied.')});
 $('settingsBtn').addEventListener('click',()=>{closeMobileNav();$('exportStatus').textContent='';renderSettingsManager();$('settingsBackdrop').classList.add('show')});$('manageAreaSelect').addEventListener('change',e=>$('renameAreaInput').value=e.target.value);$('manageTagSelect').addEventListener('change',e=>$('renameTagInput').value=e.target.value);$('renameAreaBtn').addEventListener('click',renameAreaFromSettings);$('renameTagBtn').addEventListener('click',renameTagFromSettings);$('closeSettings').addEventListener('click',()=>close('settingsBackdrop'));$('exportJson').addEventListener('click',exportJson);$('exportCsv').addEventListener('click',exportCsv);$('exportHtml').addEventListener('click',exportHtml);$('exportArchive').addEventListener('click',exportFullArchive);
 ['captureBackdrop','editBackdrop','accountBackdrop','deviceBackdrop','settingsBackdrop'].forEach(id=>$(id).addEventListener('click',e=>{if(e.target===$(id))close(id)}));document.addEventListener('keydown',e=>{if(e.key==='Escape'){setMobileNav(false);close('captureBackdrop');close('editBackdrop');close('accountBackdrop');close('deviceBackdrop');close('settingsBackdrop');panel.classList.remove('show');closeItemMenus();hideHoverPreview()}})
}
