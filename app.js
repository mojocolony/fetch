...
function wireUI(){
 const mobileNavBtn=$('mobileNavBtn'),mobileNavClose=$('mobileNavClose'),mobileNavBackdrop=$('mobileNavBackdrop');if(mobileNavBtn)mobileNavBtn.addEventListener('click',()=>setMobileNav(!document.body.classList.contains('mobile-nav-open')));if(mobileNavClose)mobileNavClose.addEventListener('click',()=>setMobileNav(false));if(mobileNavBackdrop)mobileNavBackdrop.addEventListener('click',()=>setMobileNav(false));window.addEventListener('resize',()=>{if(!window.matchMedia('(max-width:900px)').matches)setMobileNav(false)});
 ...
 $('settingsBtn').addEventListener('click',()=>{closeMobileNav();$('exportStatus').textContent='';$('settingsBackdrop').classList.add('show')});$('closeSettings').addEventListener('click',()=>close('settingsBackdrop'));$('exportJson').addEventListener('click',exportJson);$('exportCsv').addEventListener('click',exportCsv);$('exportHtml').addEventListener('click',exportHtml);$('exportArchive').addEventListener('click',exportFullArchive);
